...
- **Progressive TTS** — long section read-aloud (especially XTTS) starts after the first ~200-character chunk; remaining chunks load into a **segment queue** behind one audio bar (`Loaded N/M · ~Xs so far`). A persistent HTML player drains newly loaded segments from a JS/localStorage queue without remounting `<audio>` on each fragment tick (kills bar flicker / boundary stutter).
